...
async def identify(ws):
    ...
    logger.info("client sub")
    logger.info("must have chilli premium to join 10m+ brainorts!!!")
...
async def message_listener(ws):
    logger.info("serching for brainrots..")
    while True:
        event = json.loads(await ws.recv())
        #logger.info(f"something wrong: {str(event)[:2000]}")
        ...
        if op_code == 10: # Hello
            ...
            if event_type == "MESSAGE_CREATE" and not server.paused:
                await message_check(event) # volt
